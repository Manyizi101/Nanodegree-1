...
import pprint
import os
os.chdir('/Users/Cinkie/Documents/Nanodegree/MongoDB/Lesson_4_Problem_Set/03-Updating_Schema')
        ...
        for line in reader:
            ...
            if line['binomialAuthority_label'] == 'NULL':
                ...
                for i in range(len(keys)):
                    line[keys[i][1]] = line.pop(keys[i][0])
                    
                # remove the parenthesis in labels
                line['label'] = line['label'].split('(')[0].strip()

                newline = {}
                newline['label'] = line['label']
                newline['binomialAuthority'] = line['binomialAuthority']
                data.append(newline)
...
def update_db(data, db):
    # YOUR CODE HERE
    for line in data:
        db.arachnid.update({"label" :line['label']},
                           {'$set':{
                               'classification.binomialAuthority':line['binomialAuthority']
                               }
                            })
