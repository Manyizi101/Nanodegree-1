...
import pprint
import os
os.chdir('/Users/Cinkie/Documents/Nanodegree/MongoDB/Lesson_3_Problem_Set/01-Auditing_Data_Quality')
...
def fix_area(area):
    ...
    # YOUR CODE HERE
    if area[0] == "{":
        a1, a2 = area.replace('{','').replace('}','').split('|')
        a1_digit = a1.split('e')[0]
        a2_digit = a2.split('e')[0]
        
        if len(a1_digit) < len(a2_digit):
            area = float(a2)
        else:
            area = float(a1)
    elif area == 'NULL':
